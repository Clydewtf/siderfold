# Siderfold backend — v0.8.2

Здесь живёт backend Siderfold. Frontend из `site/` может работать либо с
публичным API, либо с явным локальным seed-набором для разработки и демонстрации.

Стек: Python 3.11+, FastAPI, Uvicorn, PostgreSQL 16, SQLAlchemy, Alembic и
Pydantic Settings.

## Каноническая схема и provenance

Каноническая программа всегда имеет primary source. В `ProgramSource` хранится
ссылка на конкретную страницу источника и время её проверки. Темы и география
связаны с программой через отдельные таблицы, deadline и funding —
необязательные связи один-к-одному.

Статус публикации принимает одно из значений: `draft`, `published`, `archived`.
Для опубликованной или архивной программы нужен `published_at`.

Финансирование не сводится к одной сумме: `exact`, `minimum`, `maximum` и
`range` сохраняют исходную форму значения. `unknown` означает, что сумму нельзя
достоверно установить, а `not_stated` — что источник её не сообщает. Эти
состояния не считаются нулём.

Для загрузок есть отдельный контур происхождения: `IngestionRun` →
`RawCapture` → `StagedRecord` → `ReviewDecision`. `DataQualityIssue` хранит
предупреждения и ошибки staging-записи. У опубликованной программы есть ссылка
на решение review, поэтому от неё можно восстановить source, запуск, raw capture
и staging-запись.

`RawCapture` хранит только метаданные: SHA-256, исходный URL, время получения,
формат, адаптер, HTTP-метаданные и внешнюю ссылку на содержимое. Бинарные файлы
и большие ответы в PostgreSQL не сохраняются. Raw capture и решения review
неизменяемы.

Staging начинается в `received`, проходит extraction и состояния качества
`warning`/`error`, затем review; только из `review` возможен переход в
`published` или `rejected`. Недопустимые переходы отклоняет PostgreSQL. Повтор
того же входа для того же source возвращает
существующий `IngestionRun` по детерминированному fingerprint. Автоматической
публикации нет: для `Program` нужны завершённый запуск, опубликованная
staging-запись и решение `publish`.

## Локальный импорт

Bridge принимает локальный пакет `siderfold.import/v1` в JSON или CSV и
записывает его в уже существующий контур `RawCapture` → `StagedRecord`.
Для локального импорта используются существующие таблицы и ограничения.

В JSON пакет состоит из метаданных источника, capture, адаптера и массива
записей:

```json
{
  "contract_version": "siderfold.import/v1",
  "source": {"name": "Название", "canonical_url": "https://source.example"},
  "capture": {
    "source_url": "https://source.example/export",
    "received_at": "2026-08-30T12:00:00+00:00",
    "external_content_uri": "file:///absolute/path/package.json",
    "content_format": "application/json"
  },
  "adapter": {"name": "local-export", "version": "1.0.0"},
  "records": [
    {
      "record_key": "source:program:example",
      "title": "Название программы",
      "record_url": "https://source.example/program/example",
      "deadline_on": "2026-12-31",
      "funding": {
        "value_kind": "maximum",
        "currency_code": "RUB",
        "max_amount": "1000000"
      },
      "payload": {},
      "warnings": []
    }
  ]
}
```

`funding` необязателен. Если он указан, его значения обязаны соответствовать
`value_kind`: `exact`, `minimum`, `maximum`, `range`, `unknown` или
`not_stated`. Тем самым отсутствующая или неопределённая сумма не превращается
в ноль.

CSV содержит одну запись на строку. Метаданные пакета повторяются в каждой
строке и должны быть одинаковы. Порядок колонок не важен, но набор фиксирован:

```text
contract_version,source_name,source_canonical_url,capture_source_url,received_at,
external_content_uri,content_format,adapter_name,adapter_version,record_key,title,
record_url,deadline_on,funding_value_kind,currency_code,exact_amount,min_amount,
max_amount,payload_json,warnings_json
```

Поля `payload_json` и `warnings_json` содержат соответственно JSON-объект и
JSON-массив строк. Неверный пакет целиком возвращает `package_errors` и не
пишется в БД. Неверная отдельная строка создаёт staging-запись в состоянии
`error` с `DataQualityIssue`; корректные строки того же пакета доходят до
`review`.

Идемпотентность определяется fingerprint нормализованного пакета вместе с
source и версией адаптера. Идентичная повторная доставка возвращает прежний
`IngestionRun` и статусы `skipped`, не создавая второй raw capture или staging
записи. Изменённая запись с тем же `record_key` создаёт следующую staging-версию
и получает статус `updated`; предыдущее происхождение сохраняется.

Bridge не создаёт `Program` и `ReviewDecision`. Даже полностью корректный
импорт остаётся набором кандидатов до отдельного review.

Команда печатает JSON-отчёт версии `siderfold.import-report/v1`. В нём есть
`dry_run`, идентификаторы source/run/raw capture, счётчики `new_count`,
`updated_count`, `skipped_count`, `error_count`, массив `rows` и
`package_errors`. У строки указаны номер, `record_key`, статус, идентификатор
staging-записи, при обновлении предыдущая staging-запись и ошибки валидации.

После настройки `DATABASE_URL` и `alembic upgrade head` импорт можно выполнить
так:

```bash
python -m app.import_bridge.cli tests/fixtures/imports/potanin_import_v1.json --format json --dry-run
python -m app.import_bridge.cli tests/fixtures/imports/potanin_import_v1.csv --format csv --dry-run
```

Для локального JSON-вывода Потанина предусмотрен узкий адаптер только для
dry-run. Он читает файл на месте, не копирует его, не сохраняет крупное
содержимое в PostgreSQL и исключает поля контактов и полного текста из staging
payload:

```bash
python -m app.import_bridge.cli /path/to/competitions.json --format potanin-json --dry-run
```

Для контрактов JSON и CSV без `--dry-run` транзакция фиксируется; использовать
этот режим следует только после review отчёта.

## Внутренний read API

Read API имеет версию `/api/v1` и доступен только для чтения:

| Метод и путь | Назначение |
| --- | --- |
| `GET /api/v1/programs` | Список опубликованных программ |
| `GET /api/v1/programs/{program_id}` | Карточка опубликованной программы |
| `GET /api/v1/sources` | Источники, связанные с опубликованными программами |
| `GET /api/v1/filters` | Доступные источники, темы, география, виды финансирования и границы сроков |

Список программ принимает `page` (с 1), `page_size` (1–100, по умолчанию 20),
`sort` (`published_at`, `updated_at`, `deadline`, `title`, `relevance`), `order`
(`asc`, `desc`), `q`,
повторяемые `source_id`, `theme`, `geography`, `funding_kind`, а также
`deadline_from` и `deadline_to`. Значения внутри одного фильтра объединяются
через OR, разные фильтры — через AND. Сортировка всегда дополнительно
стабилизируется по `id`; пустая страница возвращает `200` с пустым `items`.
Список источников использует такую же пагинацию и поддерживает сортировку по
`name` или `program_count` и поиск по имени.

Параметр `q` ищет только по названию программы. PostgreSQL приводит русские
словоформы к общим лексемам, поэтому запросы вроде «грантовой программы» находят
«Грантовая программа». Для слов длиной от четырёх символов дополнительно
применяется строгий триграммный поиск с фиксированным порогом `0.55`: он ловит
обычные опечатки, но не подменяет выдачу догадками. Все указанные слова должны
совпасть морфологически или пройти fuzzy-проверку. `sort=relevance` доступен
только вместе с `q` и всегда использует `order=desc`; точные морфологические
совпадения идут перед fuzzy-совпадениями.

Все четыре endpoint-а работают только с `Program.publication_status = published`.
`draft` и `archived` не попадают в списки, источники или фильтры; запрос карточки
для такой программы возвращает тот же `404 program_not_found`, что и неизвестный
ID. Ответы формируются отдельными Pydantic-схемами, а не ORM-моделями.

Публичные ответы содержат только канонические поля программы, источника,
deadline, funding, тем и географии. В них намеренно отсутствуют `RawCapture`,
`StagedRecord`, `DataQualityIssue`, тексты предупреждений, `ReviewDecision`,
`candidate_payload`, fingerprint, данные адаптера и внутренние provenance ID.

Успешный список имеет форму `items`, `page`, `page_size`, `total`. Ошибки имеют
форму `{ "error": { "code", "message", "details" } }`: `422 invalid_request`,
`404 program_not_found`, `503 database_unavailable` и `500 internal_error`. Полная схема доступна в
генерируемом OpenAPI (`/docs` и `/openapi.json`).

## Операторская модерация

Операционные маршруты находятся под `/api/internal/v1` и не входят в публичный
OpenAPI. Они отключены, пока в окружении нет `INTERNAL_API_TOKEN`; после
настройки каждый запрос должен передавать его как `Authorization: Bearer …`.
`INTERNAL_OPERATOR_ID` задаёт подпись оператора в audit trail. Оба значения
задаются только локально или через среду развёртывания и не хранятся в
репозитории.

Оператор может просматривать каноническую и discovery-очереди, источники,
запуски адаптеров (`/runs`), import-батчи (`/ingestion-runs`) и проблемы качества.
Канонический кейс принимает действия
`accept`, `reject`, `merge` и `needs_clarification`; URL из discovery-очереди
можно связать с уже разрешённым адаптером, отклонить или оставить на уточнении.
`POST /api/internal/v1/review/cases/{id}/revisions` сохраняет отдельную версию
ручных правок. Она не переписывает `RawCapture` или `StagedRecord`: новая версия
хранит только изменённые поля, причину, оператора, снимок дедупликации и явные
подтверждения исправленных ошибок качества. При принятии используются последние
сохранённые правки, а исходная запись остаётся доступной для сверки.
Каждый POST требует отдельный заголовок `Idempotency-Key`: повтор того же
запроса возвращает сохранённый результат без второго действия, а повторное
использование ключа с другим содержимым отклоняется.

Опубликованную программу можно архивировать через
`POST /api/internal/v1/programs/{program_id}/archive`: она сразу пропадает из
public API, но её доказательства и время первой публикации сохраняются.
Повторная публикация доступна только для такой архивированной программы и
использует `POST /api/internal/v1/programs/{program_id}/republish`. Оба действия
создают отдельную неизменяемую запись с причиной, оператором и ссылкой на
исходное решение; raw-содержимое и локальные URI файлов не отдаются ни одним
internal endpoint-ом.

Для локальной работы есть отдельная операторская страница
`http://127.0.0.1:4191/operator.html` (порт меняется через
`SIDERFOLD_FRONTEND_PORT`). Она не входит в публичный каталог и не использует
public API: после запуска `./scripts/start-local.sh api` оператор вводит тот же
`INTERNAL_API_TOKEN` в форме страницы. Токен остаётся только в памяти текущей
вкладки и не записывается в браузер, код или сборку. Страница показывает очереди,
проблемы качества, журнал запусков и безопасные действия review; сбор источников
намеренно остаётся явной CLI-командой.

В деталях кандидата есть три представления: **Данные источника** показывает все
структурированные поля, которые вернул адаптер, и безопасную цепочку
происхождения; **Предпросмотр карточки** применяет те же публичные правила,
которые будут использованы при публикации; **Правки оператора** создаёт новую
аудируемую версию, не изменяя исходный результат парсинга. «Запросить уточнение»
остаётся отдельным решением очереди, а не способом редактирования данных.

В **Правках оператора** можно исправлять все публичные части карточки, включая
график, разбивку финансирования, документы, победителей, официальные каналы и
дополнительные разделы. Ссылка подачи показывается отдельной кнопкой, а ссылки,
встречающиеся только внутри обычного текста источника, не выводятся отдельными
материалами без явного решения оператора. Первичный URL, raw capture и контакты
остаются неизменяемыми доказательствами происхождения.

После запуска backend с `INTERNAL_API_TOKEN` недавние import-батчи можно
посмотреть так:

```bash
curl -sS -H "Authorization: Bearer $INTERNAL_API_TOKEN" \
  http://127.0.0.1:8000/api/internal/v1/ingestion-runs | python3 -m json.tool
```

Очередь и один кейс удобнее смотреть через краткое read-only представление в
терминале. Оно использует только локальный backend и тот же `INTERNAL_API_TOKEN`:

```bash
python -m app.review.cli list
python -m app.review.cli show <review_case_id>
```

Пошаговый операторский порядок — от dry-run источника до проверки публикации в
каталоге — находится в [RUNBOOK.md](RUNBOOK.md).

## Реестр источников и адаптеры

Реестр хранится в `config/sources.toml` и содержит только операционные данные:
канонический URL, разрешённые префиксы и точные URL, способ доступа, лимиты,
расписание, статус, ответственного и версию адаптера. Значения секретов в репозитории не
хранятся; при необходимости конфигурация может ссылаться на имена переменных
окружения. Другой файл реестра можно передать через `SOURCE_REGISTRY_PATH`.

Все адаптеры используют один порядок работы: `discover` находит ресурсы,
`fetch` получает содержимое, `extract` выделяет записи, `validate` проверяет их,
`report` формирует итоговую статистику. Перед fetch проверяются схема, host и
path-prefix URL. Также применяются лимиты запросов, размера ответа, общего
объёма, количества записей, времени одного HTTP-запроса и всего запуска.
`timeout_seconds` и `max_run_seconds` входят в конфигурационный контракт и
применяются сетевым адаптером; локальный fixture-адаптер сети не использует.

В репозитории есть локальный fixture-адаптер. Он не обращается в сеть и нужен для
проверки самого контура:

```bash
python -m app.sources.cli list
python -m app.sources.cli dry-run fixture-catalog
```

`dry-run` выполняет discovery, fetch, extraction и validation, но не создаёт
записи в PostgreSQL. Во время интерактивного запуска безопасные строки прогресса
пишутся в stderr, а итоговый JSON остаётся в stdout. Обычный запуск после настройки `DATABASE_URL` сохраняет
статистику в `IngestionRun`, raw-метаданные и staging-кандидатов, но не создаёт
`Program` и не публикует данные автоматически:

```bash
alembic upgrade head
python -m app.sources.cli run fixture-catalog
```

Отчёт имеет версию `siderfold.adapter-report/v1` и показывает статус, счётчики
discovery/fetch/extract/validation, ошибки, предупреждения и результат передачи
в существующий import bridge. Источники со статусом `paused` или `disabled` не
запускаются. Для управляемого локального запуска есть `siderfold-sources run` и
одноразовый `siderfold-sources schedule-once`: последний проверяет только
активные источники с cron-расписанием и сразу завершается. Все текущие сетевые
источники остаются `manual`, поэтому ничего не начинает собираться само. Правила
блокировки, retry и восстановления описаны в [RUNBOOK.md](RUNBOOK.md).

### Фонд Потанина

Первый сетевой адаптер зарегистрирован как `potanin-competitions`. Discovery
читает только официальный sitemap
`https://fondpotanin.ru/sitemap-iblock-competitions.xml`, карточки из
`https://fondpotanin.ru/competitions/` и ограниченный набор прямых материалов,
на которые карточки ссылаются: документы из `/upload/` и страницы итогов из
`/press/`. Со страницы итогов допускается ещё один переход только к документу
из `/upload/`. Лимиты запросов, размера ответа, общего объёма, времени одного
запроса и всего запуска заданы в `config/sources.toml`. Сейчас один запрос может
занимать до 45 секунд, а полный обход — до 30 минут; при достижении этого
предела результат явно помечается в отчёте.

Портал подачи заявок, формы, другие хосты и ссылки вне этих путей не
открываются. Они остаются в staging как нормализованные ссылки с причиной,
почему их не собирали. Это сохраняет контекст карточки без неограниченного
обхода сайта или отправки данных в формы.

Один запуск сохраняет отдельные raw capture для sitemap, карточек и собранных
материалов. Если один и тот же файл встречается по нескольким ссылкам, байты
сохраняются один раз по SHA-256, а все исходные ссылки остаются в метаданных и
candidate payload. В PostgreSQL записываются только метаданные и URI внешнего
raw-файла; путь для сетевых ответов задаётся `RAW_CAPTURE_DIR` (по умолчанию
`/tmp/siderfold/raw`). В dry-run внешние файлы не создаются.

В candidate payload есть два слоя: нормализованные поля карточки и
`content_inventory` с разделами страницы, их заголовками, текстом и ссылками.
Неизвестный содержательный раздел не отбрасывается: он остаётся в inventory и
создаёт предупреждение для review. Для PDF и HTML-материалов сохраняется
ограниченное извлечение метаданных, заголовков, фрагмента текста и признаков
результатов; полный исходник остаётся в raw storage. PDF без текстового слоя
помечается как сохранённый без извлечения текста, а не подменяется догадкой.

График разбирается на подписанные события: приём заявок, экспертизу, результаты,
договоры и этапы реализации. Формулировка «экспертиза заявок» не считается
новым периодом подачи. Если источник явно описывает несколько циклов, все окна
подачи остаются отдельными событиями графика, а единый `deadline_on` намеренно
не заполняется. Такой кандидат получает предупреждение `multiple_application_windows`
для проверки модератором, но даты не подменяются одной произвольной.

Поля с несколькими вариантами финансирования сохраняют каждый вариант и его
контекст в breakdown. Одна каноническая сумма при этом остаётся `unknown`, пока
модератор не выберет корректную интерпретацию. По той же причине победители и
итоги пока сохраняются как доказательные материалы кандидата, а не как
автоматически опубликованные факты.

Локальная проверка без записи в БД:

```bash
python -m app.sources.cli dry-run potanin-competitions
```

В отчёте `quality` все доли содержат числитель и знаменатель:

- `completeness` — валидные кандидаты / URL карточек, принятые из sitemap;
- `validity` — валидные кандидаты / извлечённые записи;
- `duplicate_rate` — извлечённые записи с дубликатом / извлечённые записи;
- `freshness` — карточки с валидным `lastmod` / URL карточек, а также самый
  новый `lastmod` и время capture.

Эти метрики описывают только зафиксированный sitemap и ответ страниц в одном
запуске: они не подтверждают полноту сайта или актуальность условий конкурса.
Неполные, противоречивые и неожиданные данные остаются в staging с
`DataQualityIssue`; создание `Program` не происходит автоматически.
Ошибка валидации в отчёте содержит номер строки и `resource_key` с URL исходной
карточки, поэтому её можно открыть и проверить без поиска по всему каталогу.

### Фонд Тимченко

Адаптер `timchenko-competitions` читает все три официальные вкладки конкурса:
`/contests/programs/`, `/contests/ready/` и `/contests/archive/`. Все найденные
карточки остаются в одной staging/review-очереди. `source_status` (`open` или
`closed`) и `catalog_section` сохраняются для модератора и при принятии
переносятся в программу; закрытая или архивная карточка не становится
quality-error только из-за своего статуса.

После считывания каталогов адаптер получает каждую карточку, а затем в
детерминированном порядке расходует оставшийся общий бюджет на разрешённые
материалы `/upload/` и результатные страницы `/press-center/`. Внешние формы,
социальные ссылки и application CTA не открываются. Если материалов больше
доступного byte/request/time-бюджета, карточки конкурсов всё равно сохраняются,
а материалы получают `collection_status=deferred` с объясняющей причиной.
Эквивалентные HTTP/HTTPS-ссылки на одну форму объединяются в одну CTA-ссылку с
предпочтением HTTPS, когда обе версии присутствуют в источнике.

### ФАСИЭ

`fasie-competitions` собирает не все новости сайта, а полный жизненный цикл
официальных конкурсов Фонда из четырёх взаимодополняющих публичных витрин:

- главная страница и анонимная витрина `https://online.fasie.ru/m/` — текущие
  открытые наборы и их статус. Витрина является JavaScript-shell, поэтому
  адаптер читает только её публичный JSON `get-public-common-info`, без логина,
  форм подачи или interest-gathering записей;
- `/press/fund/` с локальным ограничением `lookback_days` — запуск, продление,
  изменения условий и результаты;
- `/programs/` и страницы программ — условия, тематическое семейство, лимиты и
  таблицы статусов; эти страницы обогащают карточку конкурса, а не создают её
  дубликат;
- `/competitions/` — ограниченный архив итогов и этапов. Его строки создают
  архивные result-карточки из одного raw-capture; отдельная detail-страница
  берётся только когда она уже нужна текущей ленте или странице программы.

Карточка не исчезает после окончания приёма или публикации результатов:
`source_status` сохраняется как `open`, `upcoming`, `closed` или `completed`.
Ссылки между двумя публикациями FASIE объединяют анонс и основную карточку в
один lifecycle. Внешние рейтинги, форумы, редакционные новости и льготы для уже
существующих победителей не попадают в очередь конкурсов Фонда. Внешние формы
заявок никогда не скачиваются; материалы загружаются только с
`fasie.ru/upload/` после формирования карточки. За один запуск скачивается не
более `fasie.max_artifact_requests` материалов (сейчас 48), чтобы FASIE не
ограничил весь сбор. Между живыми HTTP-запросами выдерживается
`fasie.min_request_interval_seconds` (сейчас 2 секунды); при HTTP 403/429 обход
сразу прекращается, а не продолжает нагружать сайт. Оставшиеся файлы и файлы сверх общего byte/request-бюджета
получают `collection_status=deferred` с причиной, но карточка остаётся в review.

FASIE визуально показывает фильтр дат в `list.php`, но на 23 сентября 2026
года сервер всё ещё возвращает через него полную историю. Поэтому адаптер
обходит фактическую ленту `/press/fund/` и её pager `/press/news/` →
`/press/`, применяет `lookback_days` по датам карточек локально и прекращает
обход, только когда целая страница стала старее границы. Детали скачиваются
лишь для формальных notice-карточек и явных lifecycle-сигналов — запуска,
приёма заявок, продления, изменения условий или итогов; окончательное решение
всё равно принимается после разбора полной страницы. Полезные сигналы для
review: `deadline_missing`, `related_publication_ambiguous`,
`artifact_fetch_budget_reached`, `*_limit_reached` и `artifact_*`.

Оба источника пока имеют ручное расписание, как и Потанин. Проверка и запуск:

```bash
cd /Users/clyde/projects/siderfold/backend
export PYTHON_BIN="${PYTHON_BIN:-python}"

$PYTHON_BIN -m pytest -q \
  tests/test_fasie_adapter.py \
  tests/test_timchenko_adapter.py

$PYTHON_BIN -m app.sources.cli dry-run fasie-competitions
$PYTHON_BIN -m app.sources.cli dry-run timchenko-competitions

# Только после проверки успешных отчётов и DATABASE_URL
$PYTHON_BIN -m app.sources.cli run fasie-competitions
$PYTHON_BIN -m app.sources.cli run timchenko-competitions

$PYTHON_BIN -m app.sources.cli runs --source-key fasie-competitions --limit 5
$PYTHON_BIN -m app.sources.cli runs --source-key timchenko-competitions --limit 5
```

`dry-run` делает реальные сетевые запросы, но не пишет в PostgreSQL и не
создаёт raw-файлы. `run` сохраняет provenance, raw-capture metadata,
staging-кандидатов, quality issues и review-кейсы, но не публикует `Program`
автоматически. Полные ответы остаются в `RAW_CAPTURE_DIR`, а не в PostgreSQL.
Подробный операторский порядок, критерии остановки и добавление нового источника
описаны в [RUNBOOK.md](RUNBOOK.md).

### Telegram-discovery

`telegram-cptgrantov-discovery` читает только публичную веб-страницу канала
`@cptgrantov` — `https://t.me/s/cptgrantov`. Авторизация, cookies, приватные
каналы, формы и переходы на другие хосты не используются. Первый запуск идёт
по доступным страницам истории до окончания пагинации или лимита запросов;
последующие останавливаются на сохранённом cursor сообщения.

В PostgreSQL остаются идентификатор сообщения, постоянная ссылка, дата
публикации, время получения, короткая служебная подпись до 280 символов и
нормализованные внешние URL. Текст поста, медиа, реакции и профили не
сохраняются. Для сообщений и URL выставляется срок хранения 90 дней; очистка
просроченных discovery-данных не выполняется автоматически. Операционный журнал
запусков очищается только явной командой; raw/staging и provenance этой командой
не затрагиваются.

Если URL совпадает с allowlist активного адаптера первоисточника, он получает
маршрут `source_adapter` с ключом этого источника. Сам адаптер первоисточника в
этом запуске не вызывается. Неизвестные, неоднозначные URL и сообщения без
внешней ссылки получают `manual_review`. Telegram не создаёт `Program`,
staging-запись или решение review.

`dry-run` обращается к публичной странице, но не создаёт запись в БД. Перед
live-запуском нужно отдельно проверить доступность канала и правила платформы:

```bash
python -m app.sources.cli dry-run telegram-cptgrantov-discovery
```

В `quality` этого отчёта `completeness` равен числу новых сообщений с хотя бы
одним внешним URL, делённому на число новых сообщений; `validity` — сообщениям
без ошибок разбора, делённым на число новых сообщений; `duplicate_rate` —
повторным нормализованным URL, делённым на все найденные URL. `freshness`
показывает долю сообщений с датой публикации и наиболее новую такую дату. Эти
доли описывают только ответ Telegram в пределах лимитов запуска и не являются
заявлением о полной истории канала.

## Срезы качества

Срез качества фиксирует входные факты и итог расчёта в PostgreSQL. Он ничего не
собирает из сети, не меняет карточки программ и не принимает решений review.
Поэтому один и тот же сохранённый срез можно пересчитать позже без чтения
изменившихся строк каталога.

```bash
../.venv/bin/siderfold-analytics
```

Если editable-установка ещё не обновлена после получения кода, ту же команду
можно выполнить без переустановки пакета:

```bash
../.venv/bin/python -m app.analytics.cli
```

По умолчанию окно свежести и покрытия источников — 30 дней. Его можно явно
зафиксировать вместе со временем среза:

```bash
../.venv/bin/siderfold-analytics \
  --at 2026-09-01T12:00:00+00:00 \
  --freshness-window-days 30
```

Команда выводит идентификатор snapshot, его хеши и агрегаты. В расчёт входят
только опубликованные программы, связанные с активными реальными источниками
из реестра. Seed-данные frontend-а, fixture-источники и dry-run исключаются.

- **Freshness** — доля таких программ, у которых первоисточник наблюдался в
  заданном окне.
- **Completeness** — доля карточек с названием, источником и временем
  наблюдения, дедлайном и корректно выраженным финансированием. `unknown` и
  `not_stated` — допустимые честные значения, а не нули.
- **Conflicts** — доля нерешённых canonical review-кейсов с явно отмеченным
  конфликтом точной идентичности.
- **Source coverage** — доля активных реальных источников с успешным
  не-dry-run запуском за окно.
- **Review status** — количество и доля canonical/discovery кейсов, ожидающих
  решения или уточнения.

Это внутренняя операционная и исследовательская информация: public API и
каталог показывают лишь дату обновления и допустимую атрибуцию первоисточника.
Полнота и покрытие не означают полноту рынка или абсолютную достоверность
внешних фактов.

## Запуск API

Команды выполняются из этой папки. Виртуальное окружение можно держать вне
репозитория:

```bash
python3.12 -m venv /private/tmp/siderfold-backend-venv-py3.12
source /private/tmp/siderfold-backend-venv-py3.12/bin/activate
python -m pip install -e '.[test]'
```

`scripts/start-local.sh api` выполняет эти шаги сам и предпочитает Python 3.12,
если он установлен. Для другого интерпретатора или пути к окружению можно
задать `SIDERFOLD_PYTHON` и `SIDERFOLD_BACKEND_VENV`; в конце запуска скрипт
печатает фактически использованный путь.

Поднять PostgreSQL:

```bash
docker compose --project-name siderfold up -d db
docker compose --project-name siderfold ps
docker compose --project-name siderfold port db 5432
```

Порт БД выбирается Docker автоматически, поэтому локальный `5432` не
занимается. Имена контейнера, сети и volume относятся только к проекту
`siderfold` и не пересекаются с другими Compose-проектами.

Подставить порт из предыдущей команды и запустить API:

```bash
DB_PORT="$(docker compose --project-name siderfold port db 5432 | awk -F: '{print $NF}')"
export DATABASE_URL="postgresql+psycopg://siderfold:siderfold@127.0.0.1:${DB_PORT}/siderfold"
alembic upgrade head
uvicorn app.main:app --host 127.0.0.1 --port 8000
```

Проверить в другом окне терминала:

```bash
curl -i http://127.0.0.1:8000/healthz
curl -i http://127.0.0.1:8000/readyz
```

`/healthz` отвечает, даже если БД недоступна. `/readyz` отвечает `200`, когда
удаётся выполнить `SELECT 1`, и `503`, если подключиться к PostgreSQL нельзя.

Для быстрой проверки недоступной БД можно запустить отдельный процесс так:

```bash
DATABASE_URL="postgresql+psycopg://siderfold:siderfold@127.0.0.1:1/siderfold" \
  uvicorn app.main:app --host 127.0.0.1 --port 8001
curl -i http://127.0.0.1:8001/healthz
curl -i http://127.0.0.1:8001/readyz
```

Обычный набор без PostgreSQL-инвариантов:

```bash
python -m pytest
```

Для полной проверки миграции и SQL-ограничений используется отдельная чистая
БД. Временный Compose-проект `siderfold-test` существует только на время
тестов; основной проект остаётся `siderfold`.

```bash
POSTGRES_DB=siderfold_test \
  docker compose --project-name siderfold-test up -d db
TEST_DB_PORT="$(docker compose --project-name siderfold-test port db 5432 | awk -F: '{print $NF}')"
export TEST_DATABASE_URL="postgresql+psycopg://siderfold:siderfold@127.0.0.1:${TEST_DB_PORT}/siderfold_test"

python -m pytest
docker compose --project-name siderfold-test down --volumes --remove-orphans
```

Тесты откажутся запускать миграционный rollback, если имя БД в
`TEST_DATABASE_URL` не оканчивается на `_test`.

Проверка Compose-файла:

```bash
docker compose --project-name siderfold config
```

Если основную локальную БД нужно создать заново с нуля:

```bash
docker compose --project-name siderfold down --volumes --remove-orphans
```

Эта команда удаляет только контейнер, volume и сеть проекта `siderfold`.
