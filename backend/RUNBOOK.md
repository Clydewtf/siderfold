...
`related_publication_ambiguous`, `*_limit_reached` и `artifact_*`. Отложенный
материал (`collection_status=deferred`) не означает,
...
FASIE объединяет четыре публичных представления одной конкурсной жизни:
анонимную витрину `online.fasie.ru/m/` с текущими открытыми наборами (через её
единственный публичный JSON API `get-public-common-info`, без логина и форм),
хронологическую ленту `/press/fund/` с анонсами, продлениями и итогами,
`/programs/` с условиями и таблицами статусов, а также `/competitions/` с
архивом результатов. Страницы программ обогащают конкурс, а не создают ещё одну
карточку. Анонс, продление и результат с одинаковой детерминированной
идентичностью либо явной ссылкой FASIE-на-FASIE остаются в одном review-кейсе.

У FASIE `source_status` является информационным: `open`, `upcoming`, `closed`
или `completed`. Закрытый и завершённый конкурс не отбрасывается и проходит
обычное ручное review. Рейтинги третьих лиц, форумы, редакционные новости и
льготы для уже выбранных победителей не являются конкурсами Фонда и в очередь
не попадают. Внешние сайты и формы подачи не скачиваются. Официальные материалы
загружаются только с `/upload/`. За запуск допускается не более
`fasie.max_artifact_requests` скачиваний (сейчас 48), чтобы не спровоцировать
rate-limit FASIE. Между живыми запросами соблюдается
`fasie.min_request_interval_seconds` (сейчас 2 секунды); HTTP 403/429 останавливает
обход сразу. После исчерпания этого, общего byte- или request-бюджета
материалы получают `collection_status=deferred` с причиной, не теряя саму
карточку. Архив `/competitions/` читается как единый источник названия и даты
итогов: закрытая архивная карточка сохраняет ссылку на detail, но не требует
отдельной загрузки каждой legacy-страницы.

Поле фильтра дат на `list.php` FASIE сейчас не ограничивает серверную выдачу,
поэтому адаптер намеренно его не использует. Он сам применяет `lookback_days`
по датам карточек и останавливается после полностью устаревшей страницы.
Лента меняет маршрут pager между страницами: `/press/fund/` → `/press/news/`
→ `/press/`. Сначала отбираются только формальные notice-карточки и новости с
явными признаками lifecycle; детали обычных редакционных новостей не
скачиваются. Если dry-run показывает `malformed_feed_pager`,
`feed_page_limit_reached`, `program_page_limit_reached`,
`archive_entry_limit_reached` или `artifact_fetch_budget_reached`, не запускай
`run`, пока не проверишь официальный HTML и лимиты registry-конфига.
